...
void setup_scr_screen_main_bottom(lv_ui *ui)
{
    ...
    // 天气显示标签，分行排列
    // screen_main_label_5 - 今天天气（第一行：日期+天气+风向风力）
    ui->screen_main_label_5 = lv_label_create(ui->screen_main_cont_bottom);
    lv_obj_set_pos(ui->screen_main_label_5, 0, 2);
    lv_obj_set_size(ui->screen_main_label_5, 290, 28);
    ...
    lv_obj_set_style_pad_right(ui->screen_main_label_2, 3, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_set_style_pad_bottom(ui->screen_main_label_2, 0, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_set_style_pad_left(ui->screen_main_label_2, 3, LV_PART_MAIN|LV_STATE_DEFAULT);
